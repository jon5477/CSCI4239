...
public class CopyOfFuckThis {
	public static void main(String[] args) throws IOException {
		...
		try{
			...
			fillBuffer(clBufferA.getBuffer(), hash, hash.length());

			// get a reference to the kernel function with the name 'VectorAdd'
			// and map the buffers to its input parameters.
			CLKernel kernel = program.createCLKernel("hash_SHA1");
			kernel.putArgs(clBufferA).putArg(hash.length()).putArgs(clBufferC).putArg(elementCount);
			...
			for (int i = 0; i < stuff.length; i++) {
				stuff[i] = clBufferC.getBuffer().get(i);
			}
